...
class Map1:
    ...
    def setup(self, country):
        ...
        for line in f.readlines():
            if line[:ln] == country:
                c, self.capital, lat, lon = line.rstrip().split(',')
                break
        if lat != '':
    ...
    def draw(self):
        ...
        if self.xy is not None:
            utils.centre_blit(g.screen, self.circle, self.xy)
        s = self.country
        ...
        if self.capital != '':
            ...
            utils.text_blit(
                g.screen, s, g.font1, (x, y + g.sy(1.1)), utils.CREAM, False)
        if self.latlon is not None:
            s = self.latlon
            utils.text_blit(
                g.screen, s, g.font1, (x, y + g.sy(2)), utils.CREAM, False)
