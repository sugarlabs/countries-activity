...
def message1(screen, font, m, position, d=15):
    (cx, cy) = position
...
def display_number2(screen, n, position, font, colour=BLACK):
    (cx, cy) = position
    if pygame.font:
        text = font.render(str(n), True, colour)
...
def display_number3(screen, n, position, font, colour=BLACK):
    (x, y) = position
    if pygame.font:
        lead = 0
        if n < 10:
